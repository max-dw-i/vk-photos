...
def download_album(connection, output_path, owner_id,
                   album_title, album_size, album_id=0, tagged=True):
...
def arg_handler(args):
    ...
    :returns: dictionary {'username':      str,  VK login,
    ...
    params = {'username': args.username, 'password': args.password,
              ...
              'system_wall': args.system_wall, 'system_profile': args.system_profile,
              'system_saved': args.system_saved}

    if args.system_all:
        for k in ['system_wall', 'system_profile', 'system_saved']:
            params[k] = True

    if not (args.main or args.system_all or args.system_profile or
            args.system_saved or args.system_wall or args.tagged):
        for k in ['main', 'tagged', 'system_wall', 'system_profile', 'system_saved']:
            params[k] = True
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Download the photos from a user, group vk.com page or any particular album.",
        epilog="You'll get all the photos from the page if you don't use any positional arguments."
    )
    ...
    parser.add_argument('page_type', choices=['album', 'group', 'user'], help="page type")
    parser.add_argument('link', help="""link to an album, user page or group page
                                        (e.g. https://vk.com/album11111_111111111 or
                                        https://vk.com/id1)""")
    ...
    try:
        ...
        if args_dict['page_type'] == 'album':
            owner_id, album_id = link_parse(args_dict['link'], args_dict['page_type'])
            ...
            screen_name = link_parse(args_dict['link'], args_dict['page_type'])
        ...
        exc_type, exc_obj, exc_trace_back = sys.exc_info()
        file_name = os.path.split(exc_trace_back.tb_frame.f_code.co_filename)[1]
        print(e)
        print(exc_type, file_name, exc_trace_back.tb_lineno)
        sys.exit(1)
